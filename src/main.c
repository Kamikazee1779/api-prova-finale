...
bool parse_employees(char **word_buffer, size_t *word_capacity,size_t *touched_count, int8_t *support, int *touched,Clause **clauses, size_t *clauses_size, size_t *clauses_capacity,int **literals, size_t *literals_size, size_t *literals_capacity,HashEntry *hash_table, size_t hash_capacity)
{
    ...
    /* centro de control del estado de la palabra actual */
    ...
    *touched_count = 0;
    ...
    while ((c = fgetc(stdin)) != EOF){
        if (c == '\n' || c == '\r'){
            ...
            /* reset pero esta vez ÚNICAMENTE DE LA PALABRA */
            word_active = false;
            word_length = 0;
            sign = +1;
            hash = FNV_OFFSET_BASIS;

        }else if (tautological) { /*fast-skipping*/

        }else{
            clause_started = true;

            if (!word_active){
                word_active = true; /* Es decir, me encuentro al comienzo de una palabra */
                if (c == '-'){
                    ...
                    if (word_length + 1 >= *word_capacity){ /* reservar sitio para '\0' */
                        *word_capacity *= BASE;
                        ...
                        if (!word_buffer_tmp){
                            fprintf(stderr, "Error realloc word_buffer redimensionamiento\n");
                            return false;
                        }
